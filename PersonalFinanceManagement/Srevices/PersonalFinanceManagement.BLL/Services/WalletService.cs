...
namespace PersonalFinanceManagement.BLL.Services
{
    public class WalletService : EntityServiceBase<WalletDTO, WalletCreateDTO, Wallet>
    {
        ...
        public override async Task<WalletDTO> AddAsync(WalletCreateDTO item, CancellationToken cancel = default)
        {
            if (item is null)
            ...
            return GetItem(await _walletRepository.AddAsync(GetBase(item), cancel));
        }

        public override async Task<WalletDTO> UpdateAsync(WalletDTO item, CancellationToken cancel = default)
        {
            if (item is null)
                throw new ArgumentNullException(nameof(item));

            if (!await _walletRepository.ExistsByIdAsync(item.Id, cancel))
                throw new InvalidOperationException("Wallet not found.");

            return GetItem(await _walletRepository.UpdateAsync(GetBase(item), cancel));
        }
    }
}
